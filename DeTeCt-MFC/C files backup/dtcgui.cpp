...
//***** gets acquisition file and number of framesfrom autostakkert session file
			...
			strcpy_s(popts->filename, strdup(filename_acquisition.c_str()));
...
//outputFolder = outputFolder.replace(0, log_directory.length() + 1, "");
			...
			strcpy_s(popts->ofilename, strdup(outputfilename.c_str()));
...
//				output_log << getDateTime().str().c_str() << message2.c_str() << "\n";
			...
			try {
				...
				if (popts->dateonly) {
					...
					dtcWriteLog2(log_consolidated_directory, info, (pCapture->CaptureInfo), &logline);
					...
					dtcWriteLog2(log, info, (pCapture->CaptureInfo), &logline_tmp);
				...
				if ((strlen(popts->darkfilename) > 0) && (InStr(popts->filename, WJ_DEROT_STRING) < 0) && (InStr(popts->filename, PIPP_STRING) < 0)) {
				...
				while ((pFrame = dtcQueryFrame2(pCapture, popts->ignore, &frame_error)).data) {
					...
					if ((frame_error) != 0) {
						...
						if (nframe == 1) {
							...
							if ((strlen(popts->ofilename) > 0) && (popts->allframes)) {
...
//Modification v3.2.1 comparison first ROI with full frame: applying size ratio and 80% tolerance in transparency
						...
						if ((cm.x <= 0) || (cm.y <= 0) || (currentFrameMean <= (0.2 * firstFrameMean * pFirstFrameROIMat.rows * pFirstFrameROIMat.cols / (pGryMat.rows * pGryMat.cols)))) {
							...
							if ((pROI.tl().x < tlDeltaX) || (pROI.tl().y < tlDeltaY) ||
								...
								if ((strlen(popts->ovfname) > 0) && popts->ovtype) {
									switch (popts->ovtype) {
...
/*					pADUavgMat2 = cv::Mat(pADUavgMat.rows, pADUavgMat.cols, CV_32F);
				...
				if (strlen(popts->darkfilename) > 0) {
					if (darkfile_ok == 1) {
						pADUdarkMat.release();
						pADUdarkMat = NULL;
					}
...
//						if (!is_image_correct) {
					...
					dtcWriteLog2(log_consolidated_directory, info, (pCapture->CaptureInfo), &logline_tmp);
					// if child instance, also writes log line to global log
...
//				if (popts->filename) {
			if (strlen(popts->DeTeCtQueueFilename) > 0) {
				...
				if (!popts->parent_instance && !file_exists(CString2string((CString)popts->DeTeCtQueueFilename))) dlg.OnFileExit(); 	// exits DeTeCt if Queuefile does not exists (removed at parent exit) for a child instance. Added because of difficulty to terminate children processes when exiting parent instance
					...
					SetProcessingFileProcessedFromQueue(objectname_cstring, _T("|") + objectname + _T("|") + (CString)short_logmessage.c_str() + _T("|") + (CString)std::to_string((int)rating).c_str() + _T("|") + (CString)std::to_string(duration).c_str() + _T("|") + (CString)std::to_string(nframe).c_str() + _T("|") + (CString)std::to_string(fps_int).c_str(), tag, (CString)popts->DeTeCtQueueFilename);
		...
		if ((!((!popts->parent_instance) && ((popts->maxinstances - nbChildren - 1) < 0))) && (local_acquisition_files_list.file_list.size() == 0)) do { 
			...
			if (GetFileFromQueue(&objectname, (CString) popts->DeTeCtQueueFilename)) {
				...
				if (file_stream) {
					...
					strcpy_s(popts->filename, objectnamechar);
					file_stream.close();
...
//						dlg.OnFileOpenfile();
					...
					if (folder_object = opendir(objectnamechar)) {
						...
						strcpy_s(popts->dirname, objectnamechar);
